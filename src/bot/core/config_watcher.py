...
class ConfigWatcher:
    ...
    def _collect_old_config_values(self, config_class, new_config: Dict[str, str]) -> Dict[str, Any]:
        old_values = {}
        for key in new_config:
            if hasattr(config_class, key):
                old_values[key] = getattr(config_class, key)
        return old_values

    def _convert_config_value(self, old_val: Any, value: str):
    ...
    def _apply_config_values(self, config_class, new_config: Dict[str, str]) -> None:
        for key, value in new_config.items():
            try:
                if hasattr(config_class, key):
                    old_val = getattr(config_class, key)
                    value = self._convert_config_value(old_val, value)
    ...
    async def _reload_config(self, file_path: Path):
        ...
        try:
            ...
            _old_values = self._collect_old_config_values(Config, new_config)
            self._apply_config_values(Config, new_config)
