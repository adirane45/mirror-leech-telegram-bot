...
from .command_health_monitor import CommandHealthMonitor, CommandMetrics, CommandExecution, CommandStatus
from .. import LOGGER
...
class CommandAlertSystem:
    ...
    def configure(self, owner_id: int, alert_chat_id: Optional[int] = None, enabled: bool = True):
        """
    ...
    async def register_alerts_for_command(self, command: str, tg_client=None):
        """Register alert callbacks for a specific command"""
        if not self.enabled or not tg_client:
            return
        
        async def send_alert(cmd: str, metrics: CommandMetrics, execution: CommandExecution):
    ...
    async def register_alerts_for_all_commands(self, commands: list, tg_client=None):
