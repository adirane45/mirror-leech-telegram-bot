...
class Web3StorageProvider:
    ...
    def __init__(self, ipfs_client: Optional[IPFSClient] = None):
        ...
        self.storage_index: Dict[str, Dict[str, Any]] = {}  # filename -> metadata
        self.cid_to_metadata: Dict[str, Dict[str, Any]] = {}  # CID -> metadata
    ...
    async def store_multiple_files(
        ...
        successful = sum(
            1 for r in results
            if not isinstance(r, (Exception, BaseException)) and isinstance(r, IPFSUploadResult) and r.success
        )
        ...
        return [r for r in results if not isinstance(r, (Exception, BaseException)) and isinstance(r, IPFSUploadResult)]
    ...
    def search_by_filename(self, filename: str) -> Optional[Dict[str, Any]]:
        """Search for file metadata by filename"""
        return self.storage_index.get(filename)

    def search_by_cid(self, cid: str) -> Optional[Dict[str, Any]]:
        """Search for file metadata by CID"""
        return self.cid_to_metadata.get(cid)

    def get_all_stored_files(self) -> List[Dict[str, Any]]:
