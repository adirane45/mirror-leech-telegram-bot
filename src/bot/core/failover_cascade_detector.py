...
class FailoverCascadeDetector:
    ...
    async def _cascade_monitor_loop(self) -> None:
        ...
        while self.enabled:
            try:
                await self._check_component_cascades()
                self._clear_resolved_cascades()
