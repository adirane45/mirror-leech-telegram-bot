...
class ThumbnailCache:
    ...
    def set(self, file_path: str, thumb_path: Path, dimensions: Optional[Tuple[int, int]] = None) -> None:
        """
        Cache a thumbnail
...
class SmartThumbnailManager:
    ...
    def __new__(cls, cache_dir: str = "data/thumbnails") -> 'SmartThumbnailManager':
        """Singleton __new__ method - must accept same params as __init__"""
        if cls._instance is None:
            cls._instance = super().__new__(cls)
        return cls._instance

    def __init__(self, cache_dir: str = "data/thumbnails") -> None:
        if hasattr(self, '_initialized'):
            return

        self._initialized = True
        self.cache = ThumbnailCache(cache_dir)
        self.generation_tasks: Dict[str, asyncio.Task[Optional[Path]]] = {}
    ...
    def _generate_thumbnail_sync(self, file_path: str, width: int, height: int) -> Optional[Path]:
        ...
        try:
            ...
            thumb_name = f"{file_path_obj.stem}_{width}x{height}.jpg"
    ...
    async def generate_video_sprite(
        ...
        """
        f"{file_path}_sprite_{grid_cols}x{grid_cols}"
    ...
    def _generate_sprite_sync(
        ...
        try:
            ...
            file_path_obj = Path(file_path)
            sprite_path = self.cache.cache_dir / f"{file_path_obj.stem}_sprite.jpg"
