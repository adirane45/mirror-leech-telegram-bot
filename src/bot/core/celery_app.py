...
task_decorator = cast(Callable[[F], F], celery_app.task(bind=True, max_retries=3))


@task_decorator
def test_task(self: Any, param: str) -> str:
...
# Event handlers
after_configure_handler = cast(
    Callable[[F], F],
    celery_app.on_after_configure.connect,
)


@after_configure_handler
def setup_periodic_tasks(sender: Any, **kwargs: Any) -> None:
    """Setup periodic tasks after Celery configuration"""
    LOGGER.info("✅ Celery periodic tasks configured")


after_finalize_handler = cast(
    Callable[[F], F],
    celery_app.on_after_finalize.connect,
)


@after_finalize_handler
def final_setup(sender: Any, **kwargs: Any) -> None:
