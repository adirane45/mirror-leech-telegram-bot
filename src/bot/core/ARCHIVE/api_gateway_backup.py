...
class ApiGateway:
    ...
    def _failed_response(self, request: ApiRequest, status_code: int, body: str) -> ApiResponse:
    ...
    async def _check_rate_limit_or_error(self, request: ApiRequest) -> Optional[ApiResponse]:
        if await self._check_rate_limit(request.client_id):
            return None
    ...
    def _resolve_route_or_error(self, request: ApiRequest) -> tuple[Optional[RouteConfig], Optional[ApiResponse]]:
        route = self.routes.get(request.path)
        if route:
            return route, None
        return None, self._failed_response(request, 404, "Route not found")
    ...
    async def _check_auth_or_error(self, request: ApiRequest, route: RouteConfig) -> Optional[ApiResponse]:
        ...
        if await self._check_auth(auth_token):
            return None
        return self._failed_response(request, 401, "Unauthorized")
    ...
    async def _select_target_or_error(self, request: ApiRequest, route: RouteConfig) -> tuple[Optional[str], Optional[ApiResponse]]:
        ...
        return None, self._failed_response(request, 503, "No available nodes")

    async def _check_circuit_or_error(self, request: ApiRequest, target_node: str) -> Optional[ApiResponse]:
        if await self._check_circuit(target_node):
            return None
    ...
    async def _notify_request_routed(self, request: ApiRequest, target_node: str) -> None:
        for listener in self.listeners:
            await listener.on_request_routed(request, target_node)

    async def _finalize_response_metrics(
        self,
    ...
    async def route_request(self, request: ApiRequest) -> ApiResponse:
        ...
        try:
            ...
            if rate_error := await self._check_rate_limit_or_error(request):
                return rate_error

            route, route_error = self._resolve_route_or_error(request)
            if route_error:
                return route_error
            ...
            await self._notify_request_routed(request, target_node)

            # Process request (simulated)
            response = await self._process_request(request, target_node)

            await self._finalize_response_metrics(response, target_node, start_time)
