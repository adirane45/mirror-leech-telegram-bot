...
class FailoverManager:
    ...
    async def _execute_recovery_attempts(self, operation: RecoveryOperation, action: RecoveryAction) -> bool:
    ...
    async def _update_operation_metrics(self, operation: RecoveryOperation, success: bool) -> None:
        """Update metrics after operation completion"""
        if success:
    ...
    async def _notify_listeners_started(self, operation_id: str) -> None:
        """Notify listeners that recovery started"""
        for listener in self.listeners:
            await listener.on_recovery_started(operation_id)
    
    async def _notify_listeners_completed(self, operation_id: str, success: bool) -> None:
        """Notify listeners that recovery completed"""
        for listener in self.listeners:
            await listener.on_recovery_completed(operation_id, success)
    ...
    async def _execute_recovery(self, action: RecoveryAction, concurrent_id: int) -> None:
        ...
        try:
            ...
            await self._notify_listeners_started(operation.operation_id)
            success = await self._execute_recovery_attempts(operation, action)
            await self._update_operation_metrics(operation, success)
            await self._notify_listeners_completed(operation.operation_id, success)
    ...
    async def _cascade_monitor_loop(self) -> None:
        ...
        def has_recent_failures(failures) -> bool:
            now = datetime.now(UTC)
            recent = [f for f in failures if now - f < timedelta(seconds=10)]
            return bool(recent)

        async def check_component_cascades() -> None:
            for component_id in list(self.component_failures.keys()):
                failures = self.component_failures[component_id]
                if has_recent_failures(failures):
                    await self.detect_cascading_failure(component_id)

        def clear_resolved_cascades() -> None:
            now = datetime.now(UTC)
        ...
        while self.enabled:
            try:
                await check_component_cascades()
                clear_resolved_cascades()
