...
from ... import LOGGER, intervals, nzb_jobs, nzb_listener_lock, sabnzbd_client
...
@new_task
async def _process_history_job(job, nzo_id):
    """Process a single history job"""
...
async def _process_download_job(dl, nzo_id):
        ...
        async with nzb_listener_lock:
            try:
                ...
                for job in jobs:
                    nzo_id = job["nzo_id"]
                    if nzo_id in nzb_jobs:
                        await _process_history_job(job, nzo_id)
                
                for dl in downloads:
                    nzo_id = dl["nzo_id"]
                    if nzo_id in nzb_jobs:
                        await _process_download_job(dl, nzo_id)
