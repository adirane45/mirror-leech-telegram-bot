...
from bot.helper.telegram_helper.message_utils import edit_message, send_file, send_message
...
def _build_thumbnail_path(file_path: str, file_name: str) -> str:
    file_dir = os.path.dirname(file_path) or "."
    file_base = os.path.splitext(file_name)[0]
    return os.path.join(file_dir, f"{file_base}_thumbnail.jpg")
...
async def extract_thumbnail(_, message: Message):
    ...
    try:
        thumbnail_path = _build_thumbnail_path(file_path, file_name)
        ...
        if success and thumbnail_exists:
