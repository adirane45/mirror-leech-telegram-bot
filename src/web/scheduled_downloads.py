...
class PriorityQueue:
    ...
    def add(
        self, download_id: str, priority: int = 5, url: str = "", operation: str = ""
    ) -> None:
    ...
    def get_priority(self, download_id: str) -> int | None:
    ...
    def set_priority(self, download_id: str, new_priority: int) -> bool:
        ...
        with self.lock:
            ...
            # Find and remove
            self.queue = [
                (p, c, item) for p, c, item in self.queue if item["id"] != download_id
            ]
            ...
            updated_item["priority_changed"] = datetime.now().isoformat()
            heapq.heappush(self.queue, (new_priority, self.counter, updated_item))
            return True
...
class ScheduledDownloads:
    ...
    def get_scheduled_downloads(self, status: str | None = None) -> list[dict[str, Any]]:
    ...
    def dequeue_download(self) -> dict[str, Any] | None:
