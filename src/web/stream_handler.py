...
RETRY_MAX_ATTEMPTS = int(environ.get("STREAM_RETRY_MAX_ATTEMPTS", "3"))
RETRY_MAX_WAIT_SECONDS = float(environ.get("STREAM_RETRY_MAX_WAIT_SECONDS", "2.0"))
...
async def _call_with_retry(label, func, *args, **kwargs):
    try:
        ...
        LOGGER.warning("stream_retry_failed op=%s error=%s", label, e)
        raise
...
async def _stream_file(file_url: str):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            resp = await _call_with_retry("telegram.cdn", session.get, file_url)
            async with resp:
                if resp.status != 200:
                    body = await resp.text()
